...
export function ResetPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!tokenReady) {
      setMsg(
        "再設定トークンが見つかりません。メールのリンクを開き直してください。",
      );
      setDone(false);
      return;
    }
      ...
      <div className="mx-auto max-w-[960px]">
        <section className="rounded-[36px] border border-[#e6d9ce] bg-white px-6 py-8 shadow-[0_10px_28px_rgba(110,78,56,0.08)] md:px-8 md:py-10">
          ...
          <p className="mb-8 text-base font-semibold leading-8 text-[#766b63]">
            メールに記載されたリンクから開いた場合、そのまま新しいパスワードを設定できます。
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {done ? (
              <Link
                to="/login"
                className="inline-flex rounded-full bg-[#4e342e] px-4 py-2.5 text-sm font-bold text-white transition hover:opacity-90"
              >
                ログインへ
              </Link>
            ) : null}
